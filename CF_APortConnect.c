...
int CF_APortConnect(int Type, long* pInfo)
{
  ...
  for( ; i<SessCnt+idx; i++ )
  {
    ...
    memset( g_TermTable[i].SGName, 0x00, 
        sizeof(g_TermTable[i].SGName));
    strncpy( g_TermTable[i].SGName,
        pResult->SrcSvc, strlen(pResult->SrcSvc) );
...
#if 0
      if( (g_UsePoll == 1) && (g_TermTable[i].cnt_stat == 1) )
      {
        // On Timer
        lParam = g_TermTable[i].stidx;
...
int func_ASearchEmptyTbl()
{
  ...
  for(i=0; i<g_maxterm_cnt; i++)
  {
    ...
    {
#if 0
      g_TermTable[i].onuse		= 2;
      g_TermTable[i].stidx		= -1;
      g_TermTable[i].cnt_stat		= -1;

      g_TermTable[i].PeerIP		= 0;
      g_TermTable[i].PeerPort		= 0;
      g_TermTable[i].LocalPort	= 0;
...
void func_AClearTable(int idx)
{
...
#if 0
  int nRc; 
  nRc = rsof_resetFD(g_TermTable[idx].stidx, _IGNORE, _IGNORE);
  if(nRc < 0)
  {
    ulog(_ERROR_, "Session Close Fail.(%d)", nRc);
  }

  ulog(_FLOW_, "Session Close idx:%d\n", idx);
#endif
  ...
  memset( g_TermTable[idx].SGName, 0x00, 
      sizeof(g_TermTable[idx].SGName) );


  //    g_TermTable[idx].poll_cnt	= 0;
  //	g_TermTable[idx].sndcnt		= 0;
  //	g_TermTable[idx].rcvcnt		= 0;
...
int	func_SendChannelStat(int stat)
{
  ...
  if(stat == 1)
  {
    memcpy( pHdr->c_MsgDsc, DEF_MSG_DSC_UPCHANNEL
        , strlen(DEF_MSG_DSC_UPCHANNEL));
  }
  else
  {
    memcpy( pHdr->c_MsgDsc, DEF_MSG_DSC_DOWNCHANNEL
        , strlen(DEF_MSG_DSC_DOWNCHANNEL));
  }
