/******************************************************************************
 화일명     : tb_clc_svc_flow_mst_ct.h
 내용설명   : TB_CLC_SVC_FLOW_MST c용 Structure
 참고 사항  :
 변경이력   :
     일자             작성자                        내용
  ----------         --------     ---------------------------------------
  2012.04.12                  초기 작성
******************************************************************************/
#ifndef	__TB_CLC_SVC_FLOW_MST_CT_H__
#define	__TB_CLC_SVC_FLOW_MST_CT_H__

int  db_Insert_tb_clc_svc_flow_mst(char *p_tstr);
int  db_Select_tb_clc_svc_flow_mst(char *p_tstr);
int  db_Update_tb_clc_svc_flow_mst(char *p_tstr);
void db_Disp_tb_clc_svc_flow_mst(char *p_tstr);
void db_Init_tb_clc_svc_flow_mst(char *p_tstr);

typedef	struct
{
	struct { unsigned short len; unsigned char arr[ 20+1]; } svc_flow_mst_dsc;
	int     svc_prc_sq;
	struct { unsigned short len; unsigned char arr[ 16+1]; } ap_svc_nm;
	struct { unsigned short len; unsigned char arr[  1+1]; } svc_kndc;
	struct { unsigned short len; unsigned char arr[  1+1]; } use_yn;
	struct { unsigned short len; unsigned char arr[256+1]; } svc_flow_mst_nm;
	struct { unsigned short len; unsigned char arr[ 10+1]; } chan_bsn_dept_c;
} TB_CLC_SVC_FLOW_MST_CT;

#endif
