...
int UserInit()
{
  ...
  if(g_UsePoll == 1)
  {
    ...
    {
      ulog(_WARNING_, "Config 설정 오류(value:%d) : [PollInfo] PollInterval -> default 30분으로 설정", g_ReqPollInterval);
    ...
    {
      ulog(_WARNING_, "Config 설정 오류(value:%d) : [PollInfo] PollTimeOut -> default 30 sec로 설정", g_RspPollInterval);
