...
int cli_send_1(char *PeerIP, char *PeerPort, char *LocalPort) {
  ...
  if (g_Encrypt_Flag == 0) {
    ulog(_ERROR_, "[장애정보] 암호화 사용 안함");
    return -1;
  }

  ulog(_FLOW_,
       "[로그정보] Client 세션키 교환 요구(1) 송신 준비"
       "[상세정보] %s->%s:%s",
  ...
  if (rc != 0) {
    ulog(_ERROR_, "INL_Handshake_Init CLIENT Failed. errcode=%d msg[%s]", rc,
         INL_ErrorString(rc));
    if (pSKeyOut != NULL)
      INL_Free_Buf(pSKeyOut);
    return -3;
  }

  ulog(_FLOW_,
       "[로그정보] MSG 000000001 DATA부\n"
       "[상세정보] (len:%d)[%s]\n",
  ...
  if (rc < 0) {
    ulog(_ERROR_, "RMP error(%d) Session Key Request(1) len(%d)", rc,
         g_rmpSvcName, msg_len);
    return -1;
  }
...
int cli_recv_2(char *msg, int len) {
  ...
  /* 암호화 정보 취득 */
  rc = INL_Handshake_Update(g_client_ctx, pKey, strlen((char *)pKey), &pSKeyOut,
                            &out_len);
  if (rc != 0) {
    ulog(_ERROR_,
         "[장애정보] INL_Handshake_Update Failed\n"
...
int cli_recv_4(char *msg, int len) {
  ...
  /* 암호화 정보 취득 */
  rc = INL_Handshake_Final(g_client_ctx, pKey, strlen((char *)pKey), &pSKeyOut,
                           &out_len);
  if (rc != 0) {
    ulog(_ERROR_,
         "[장애정보] INL_Handshake_Final Failed\n"
         "[상세정보] errcode[%d], msg[%s]",
         rc, INL_ErrorString(rc));
    if (pSKeyOut != NULL)
      INL_Free_Buf(pSKeyOut);
  ...
  rc = rmp_MessageProc(g_rmpSvcName, 0, pMsg, msg_len, &Info1, &Info2);
  if (rc < 0) {
    ulog(_ERROR_, "RMP error(%d) (CLI)Session Key Request(5) len(%d)", rc,
         msg_len);
    return -1;
  }
  ...
  if (g_UsePoll == 1) {
    ...
    rdf_setTimer(TIMERID_REQ_POLL, g_Poll_RampUp * 1000, -1, 0, 0,
                 TF_SendPollReq);
  } else {
